...
public class ProyectoDAO {
    ...
    public boolean actualizarProyecto(Proyecto proyecto) {
        String sql = "UPDATE proyecto SET nombre = ?, descripcion = ?, estatus = ?, fk_empresa = ? "
                   + "WHERE id_proyecto = ?";
        ...
        String sql = """
            SELECT p.id_proyecto, p.nombre, p.descripcion, p.estatus, p.fk_empresa,
             ...
             WHERE YEAR(p.fecha_registro) = ?
               AND MONTH(p.fecha_registro) BETWEEN ? AND ?
            """;
    ...
    public boolean agregarProyecto(Proyecto proyecto) {
        String sql = "INSERT INTO proyecto (fk_empresa, nombre, descripcion, estatus) "
                   + "VALUES (?, ?, ?, ?)";
        try (Connection conn = Conexion.getConexion();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public Anteproyecto obtenerAnteproyectoPorNombre(String nombre) {
        String sql = ""
            + "SELECT id_anteproyecto, nombre, descripcion, fecha_registro, estado, statusant, "
            + "       fk_alumno, fk_docente, rfc_empresa "
            + "  FROM anteproyecto "
            + " WHERE nombre = ?";
        try (Connection conn = Conexion.getConexion();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    ap.setStatusant(rs.getString("statusant"));
                    ap.setFkAlumno(rs.getInt("fk_alumno"));
                    ap.setFkDocente(rs.getInt("fk_docente"));
